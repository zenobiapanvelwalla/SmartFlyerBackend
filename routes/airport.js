...
router.post('/getOne', function (req, res) {
  ...
  Airport.findOne({
    ...
    let sum = 0;
    if (airport) {
      for (var index = 0; index < airport.waittimes.length; index++) {
        sum += airport.waittimes[index].wait
        count += 1
      }
      airport.averageWaitTime = isNaN(sum / count) ? "0" : (sum / count).toFixed(2).toString()
    }
    res.json(airport);
  });
});
...
router.post('/search', function (req, res) {
  ...
  Airport.find({
    ...
    for (let i = 0; i < result.data.length; i++) {
      ...
      result.data[i].averageWaitTime = isNaN(sum / count) ? "0" : (sum / count).toFixed(2).toString()
    }
    res.json(result)
  });
});
...
router.post('/getNearest', function (req, res) {
  ...
  for (let i = 0; i < AllAirports.length; i++) {
    if (AllAirports[i].country == 'United States') {
      ...
      AllAirports[i].distance = d
      if (d < 150) {
        nearestAirports.push(AllAirports[i])
      }
    }
  }
  nearestAirports.sort((a, b) => (a.distance > b.distance) ? 1 : ((b.distance > a.distance) ? -1 : 0));
  let result = {
    data: nearestAirports
  }
  if (nearestAirports.length > 20) {
    result.data = nearestAirports.slice(0, 19)
  }
  res.json(result)
});
...
router.get('/saveAirportImage', function (req, res) {
  ...
  for (let i = 0; i < AllAirports.length; i++) {
    let st1 = AllAirports[i].city.replace(/[^a-zA-Z ]/g, "").toLowerCase()
    for (let j = 0; j < imageResource.length; j++) {
      let st2 = imageResource[j].name.replace(/[^a-zA-Z ]/g, "").toLowerCase()
